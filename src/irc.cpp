...
void ThreadIRCSeed(void* parg)
{
    ...
    RenameThread("AtroxCoin-ircseed");
...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #AtroxCoinTEST2\r");
            Send(hSocket, "WHO #AtroxCoinTEST2\r");
        } else {
            // randomly join #AtroxCoin00-#AtroxCoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #AtroxCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #AtroxCoin%02d\r", channel_number).c_str());
        }
