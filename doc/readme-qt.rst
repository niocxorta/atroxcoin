AtroxCoin-qt: Qt4 GUI for AtroxCoin
===============================
...
Alternatively, install Qt Creator and open the `AtroxCoin-qt.pro` file.

An executable named `AtroxCoin-qt` will be built.
...
A warning for people using the *static binary* version of AtroxCoin on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of AtroxCoin is linked against libdb5.3.
...
installed causes AtroxCoin-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
